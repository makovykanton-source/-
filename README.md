...
### Проблема: `Type "Redis" is not assignable to type "ConnectionOptions"` (BullMQ/ioredis)

Это конфликт типов из разных копий `ioredis` (корневая и вложенная в `bullmq`).

Решение:

1. Убедитесь, что очереди/воркеры используют `bullmqConnection`, а не `redis` instance напрямую:
   - `src/infrastructure/queue/queues.ts`
   - `src/workers/main.ts`
2. Очистите зависимости и установите заново:
...
### Проблема: `Cannot find module dist/...` при `npm run start|bot|worker|migrate`

`dist` не собран (или `build` упал ранее на type-check).
...
Для разработки без сборки используйте:
...
### Проблема: `ZodError` с обязательными переменными (`DISCORD_TOKEN`, `DATABASE_URL` и т.д.)

`.env` не заполнен. Скопируйте шаблон и задайте реальные значения:
...
для разработки без `dist`: `npm run api:dev`, `npm run bot:dev`, `npm run worker:dev`)
...
Если прокси вам **не нужен**, используйте готовые scripts — они кроссплатформенно очищают proxy env (`HTTP(S)_PROXY`, `ALL_PROXY`, `NO_PROXY`, `npm_config_*`) только на время команды:
...
Если нужен ручной запуск, удалите proxy env и выполните обычный `npm install`.
